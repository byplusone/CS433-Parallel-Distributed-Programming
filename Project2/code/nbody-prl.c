...
void position_step(struct world *world, double time_res) {
    ...
    #pragma omp parallel for private(j) /*schedule(dynamic)*/ //reduction(+: world->bodies[i].vx, +: world->bodies[i].vy, +: world->bodies[i].x, +: world->bodies[i].y) //schedule(dynamic) reduction(world->bodies[i].vx, world->bodies[i].vy, world->bodies[i].x, world->bodies[i].y)
    for (i = 0; i < world->num_bodies; i++) {
  ...
  // Update the velocity and position of each body
	//#pragma opm parallel for reduction(+: world->bodies[i].vx, +: world->bodies[i].vy, +: world->bodies[i].x, +: world->bodies[i].y) //schedule(dynamic)
    for (i = 0; i < world->num_bodies; i++) {
        // Update velocities
        world->bodies[i].vx += force_x[i] * time_res / world->bodies[i].m;
        world->bodies[i].vy += force_y[i] * time_res / world->bodies[i].m;		
